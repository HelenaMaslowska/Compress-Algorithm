...
*/ 
    ...
    while (strip)
    {
            ...
            for (uint32_t k = 1; k < strip_size - 1; k++, curr_index++)
            {
                ...
                */
                ...
                else if (!memcmp(&prev, &curr, sizeof(cd_color_t)) && memcmp(&curr, &next, sizeof(cd_color_t)))    //else if (prev == curr && curr != next)
                {
                    ...
                    encode_raw(buf_screen, buf_encoded, *size_buf_encoded, &curr_size_bytes, if_index, first_index, same);
        ...
        if (last_pixels == 1)
        {
            // printf("-----------------------------------------------------------------------------\n");
            not_same++;
            encode_raw(buf_screen, buf_encoded, size_buf_encoded, &curr_size_bytes, if_index, curr_index-1, not_same); // prev to RAW
